...
public interface RelationRepository extends PagingAndSortingRepository<Relation, Long>{
	...
	public List<RegisteredUser> getfriendsbyuserid(@Param("id") long id,Pageable page);
	
	@Query("Select count(u2) from Relation r,RegisteredUser u1,RegisteredUser u2 where u1.id=:id and ((r.user1=u1 and r.user2=u2) or(r.user2=u1 and r.user1=u2)) and r.type='friends'")
	public int countfriends(@Param("id") long id);
	...
	@Query("select count(u) from RegisteredUser u WHERE u.id IN (Select u1.id from Relation r,RegisteredUser u1,RegisteredUser u2 where u2.id=:id and (r.user1=u1 and r.user2=u2)  and r.type='Poslan zahtjev')")
	
	public int getnumberofrequests(@Param("id")long id);
